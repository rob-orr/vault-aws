...
module "vault_server_sg" {
  ...
  vpc_id      = "${var.vpc_id}"
  cidr_blocks = ["${split(",", var.public ? join(",", var.public_cidrs) : var.vpc_cidr)}"] # If there's a public IP, open Vault ports for public access.
}
...
module "consul_client_sg" {
  ...
  vpc_id      = "${var.vpc_id}"
  cidr_blocks = ["${split(",", var.public ? join(",", var.public_cidrs) : var.vpc_cidr)}"] # If there's a public IP, open Consul ports for public access.
}
...
resource "aws_security_group_rule" "ssh" {
  ...
  from_port         = 22
  to_port           = 22
  cidr_blocks       = ["${split(",", var.public ? join(",", var.public_cidrs) : var.vpc_cidr)}"] # If there's a public IP, open port 22 for public access.
}
...
module "vault_lb_aws" {
  ...
  name               = "${var.name}"
  vpc_id             = "${var.vpc_id}"
  cidr_blocks        = ["${split(",", var.is_internal_lb ? var.vpc_cidr : join(",", var.public_cidrs))}"]
